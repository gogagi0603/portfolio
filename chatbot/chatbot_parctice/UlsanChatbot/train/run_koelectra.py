...
from dataloader.chatbot import ChatbotTextClassificationDataset
from model.koelectra import koElectraForSequenceClassification
import options
# by태관 epoch 한번 돌아갈때 실행
def train(epoch, model, optimizer, train_loader, save_step, save_ckpt_path, train_step = 0):
    ...
    with tqdm(total= total_train_step, desc=f"Train({epoch})") as pbar:
        ...
        for i, data in enumerate(train_loader, train_start_index):
            ...
            #by태관 모델에 input 넣고 outputs 들고옴 outputs == [(loss), (logits), (hidden_states), (attentions)]
...
if __name__ == '__main__':
    data_path = "../data/deglok_text_classification_train.txt"
    checkpoint_path ="../checkpoint"
    save_ckpt_path = f"{checkpoint_path}/deglok-text-classification.pth"
    ...
    # chatbot TextClassificationDataset 데이터 로더
    dataset = ChatbotTextClassificationDataset(tokenizer=tokenizer, device=device)
